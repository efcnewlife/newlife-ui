...
export const createTimeOfDay = (hour: number, minute: number, second = 0): Dayjs => {
  return dayjs(`${TIME_OF_DAY_ANCHOR} ${pad(hour)}:${pad(minute)}:${pad(second)}`, "YYYY-MM-DD HH:mm:ss");
};
...
export const defaultTimeFormat = (timePrecision: TimePrecision = "minutes", ampm = false): string => {
  if (ampm) {
...
export const isCompleteTimeString = (text: string, timePrecision: TimePrecision = "minutes", ampm = false): boolean => {
  if (ampm) {
    const twelveOk = timePrecision === "seconds" ? TIME_12H_SECONDS.test(text) : TIME_12H_MINUTES.test(text);
    if (twelveOk) {
      return true;
    }
    const twentyFour = timePrecision === "seconds" ? TIME_24H_SECONDS.exec(text) : TIME_24H_MINUTES.exec(text);
    if (!twentyFour) {
      return false;
    }
...
export const parseTimeString = (text: string, timePrecision: TimePrecision = "minutes", ampm = false): Dayjs | null => {
  if (ampm) {
    const twelve = timePrecision === "seconds" ? TIME_12H_SECONDS.exec(text) : TIME_12H_MINUTES.exec(text);
    if (twelve) {
...
export const formatTimeInput = (text: string, timePrecision: TimePrecision = "minutes", ampm = false): string => {
